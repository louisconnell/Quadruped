...
void loop() {
  ...
  pwm.setPWM(Coxas[1], 0, pulseWidthInverse(DegreeCoxas[1]));
  pwm.setPWM(Femurs[1], 0, pulseWidthNormal(DegreeFemurs[1]));
  pwm.setPWM(Tibias[1], 0, pulseWidthNormal(DegreeTibias[1]));
  pwm.setPWM(Coxas[2], 0, pulseWidthInverse(DegreeCoxas[2]));
  pwm.setPWM(Femurs[2], 0, pulseWidthInverse(DegreeFemurs[2]));
  pwm.setPWM(Tibias[2], 0, pulseWidthInverse(DegreeTibias[2]));
  pwm.setPWM(Coxas[3], 0, pulseWidthInverse(DegreeCoxas[3]));
  pwm.setPWM(Femurs[3], 0, pulseWidthNormal(DegreeFemurs[3]));
  pwm.setPWM(Tibias[3], 0, pulseWidthNormal(DegreeTibias[3]));
  delay(10);
  Serial.println();
}
...
void WalkingCycle() { //define what step in each leg cycle and what position each leg has to take
  if (legIndex < 96) {
    if (dirX > 0 || 0 > dirX) {
      int StepLength = dirX;
      if (WHATLEG_X == 0) {
        StepLength = map(StepLength, -100, 100, -140, 140); //defining step length in mm
      }
      if (WHATLEG_X == 1) {
        StepLength = map(StepLength, -100, 100, 140, -140);
      }
  ...
  int BodyHeight = dirZ;
  if (dirZ > 0 || 0 > dirZ) {
    BodyHeight = map(BodyHeight, -100, 100, -30, 30);
  ...
  if (legIndex < 96) { // Z COORDINATE TO SEND TO IK for keeping leg on ground
    ...
    nextZ = 0.1 * sq(legIndex - 112) - 20 + (NeutralZ + BodyHeight); // Z COORDINATE TO SEND TO IK for lift parabola
  }

  /*
    if (rotZ > 0 || 0 > rotZ) {
      Serial.print("walking cycle rotZ ");
      Serial.println(rotZ);
      float RotSpeed = map_float(rotZ, -100, 100, -PI, PI);
      Serial.println(RotSpeed);
      float RotAngle = RotSpeed * 10 / 96; //defining micro angle steps
      ...
      nextX2 = nextX + ((LegLength * cos(CurCoxaAngle + (RotAngle / 10))) - 60); // X COORDINATE TO SEND TO IK
      Serial.print("walking cycle nextX ROT ");
      Serial.println(nextX2);
      nextY2 = nextY + ((LegLength * sin(CurCoxaAngle + (RotAngle / 10))) - 60); // Y COORDINATE TO SEND TO IK
